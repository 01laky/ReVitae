...
/// <summary>Canonical maximally filled John Doe CV used by stress PDF generation and import matrix tests.</summary>
public static class JohnDoeStressCvDataset
{
	public static CvExportDocument CreateDocument(
		CvExportTemplateId templateId = CvExportTemplateId.ModernSidebar,
	...
	public static string BuildSummary()
	{
		const string paragraph =
			"Senior full stack engineer with 18+ years of experience designing distributed systems, " +
			"leading cross-functional teams, and delivering secure, observable platforms for fintech, " +
			"healthcare, e-commerce, and developer tooling. Expert in cloud-native architecture, API design, " +
			"data pipelines, and pragmatic engineering leadership.";

		var summary = paragraph;
		while (summary.Length < 760)
		{
			summary += " " + paragraph;
		}

		return summary[..Math.Min(summary.Length, 800)].TrimEnd();
	}
	...
	public static string BuildAdditionalInformation()
	{
		...
		var notes = paragraphs.ToList();
		var noteTemplate =
			"Additional note {0:D2}: Extended detail about leadership philosophy, delivery practices, architecture decision records, " +
		...
		for (var index = 1; notes.Count < 20; index++)
		{
			notes.Add(string.Format(noteTemplate, index));
		}

		var content = string.Join("\n\n", notes);
		return content.Length <= 2800 ? content : content[..2800].TrimEnd();
	}
